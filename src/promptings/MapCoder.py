...
class MapCoder(BaseStrategy):
    ...
    def xml_to_dict3(self, element):
        result = {}
        for child in element:
            # Recursively parse child if it has sub-elements
            child_data = self.xml_to_dict(child) if list(child) else child.text

            # Always store repeated tags in a list
            if child.tag in result:
    ...
    def xml_to_dict2(self, element):
        ...
        for child in element:
            ...
            if child.tag in result:
                ...
                result[child.tag] = child_data
        # Return text directly if no children and text exists
    ...
    def parse_xml2(self, response):
        parsed = xmltodict.parse(response)

        final_dict = {
            "problem": parsed["root"]["problem"],
            "algorithm": parsed["root"]["algorithm"]
        }
    ...
    def get_knowledge_base_and_exemplars(self, item: dict) -> tuple[dict, int, int]:
        input_kb_exemplars = [{
            "role": "user",
...
"""
        ...
        print(input_kb_exemplars[0]['content'])

        response, pr_tok, com_tok = self.gpt_chat(processed_input=input_kb_exemplars, index=0)
        item['api_calls'] = item.get('api_calls', 0) + 1

        # Post processing
        response = self.trim_text(response, "# Identify the algorithm (Brute-force, Dynamic Programming, Divide-and-conquer, Greedy, Backtracking, Recursive, Binary search, and so on) that needs to be used to solve the original problem.")
        response = self.trim_text(response, "# Write a useful tutorial about the above mentioned algorithms. Provide a high level generic tutorial for solving this types of problem. Do not generate code.")
        response = self.trim_text(response, "# Planning to solve this problem:")
        response = self.trim_text(response, f"# Let's think step by step to solve this problem in {self.language} programming language.")
        ...
        return self.parse_xml(response), pr_tok, com_tok

    def get_problem_planning(self, example: dict, example_no: int, item: dict, algorithm_prompt: str, sample_io_prompt: str) -> tuple[str, int, int]:
        ...
        print(input_for_problem_planning[0]['content'])

        planning, pr_tok, com_tok = self.gpt_chat(input_for_problem_planning, index=0)
        item['api_calls'] += 1
        
        return planning, pr_tok, com_tok

    def verify_planning(self, item: dict, planning: str) -> tuple[dict, int, int]:
        ...
        print(input_for_planning_verification[0]['content'])

        verification_res, pr_tok, com_tok = self.gpt_chat(input_for_planning_verification, index=0)
        item['api_calls'] += 1
    ...
    def planning(self, example, example_no, item, algorithm_prompt, sample_io_prompt) -> tuple[tuple, int, int]:
        planning, pr_tok, com_tok = self.get_problem_planning(
            example, 
            example_no,
            item,
            algorithm_prompt,
        ...
        verification_res, pr_tok_1, com_tok_1 = self.verify_planning(item, planning)
        item['api_calls'] += 1
        
        pr_tok += pr_tok_1
        com_tok += com_tok_1

        return (planning, verification_res['confidence'], example), pr_tok, com_tok

    def run_single_pass(self, item: dict):
        print("")

        kb_exemplars, pr_tok, com_tok = self.get_knowledge_base_and_exemplars(item)

        algorithm_prompt = f"## Relevant Algorithm to solve the next problem:\n{ kb_exemplars['algorithm']}"
        sample_io_prompt = f"## Sample Test cases: \n{self.get_sample_io_str(item['sample_io'])}\n"
        # if type(self.data) != MBPPDataset and type(self.data) != XCodeDataset else ""

        plannings = []
        for example_no, example in enumerate(kb_exemplars["problem"], start=1):
            planning_tuple, pr_tok_1, com_tok_1 = self.planning(
                example,
                example_no,
                item,
                algorithm_prompt,
            ...
            plannings.append(planning_tuple)
            pr_tok += pr_tok_1
            com_tok += com_tok_1
        ...
        for planning_with_ex in plannings:
            ...
            code, pr_tok_1, com_tok_1 = self.generate_final_code(item, algorithm_prompt, planning, sample_io_prompt, std_input_prompt)
            pr_tok += pr_tok_1
            com_tok += com_tok_1
            ...
            for i in range(1, self.t + 1):
                ...
                response, pr_tok_1, com_tok_1 = self.improve_code(item, algorithm_prompt, response, test_log, std_input_prompt)
                item['api_calls'] += 1
                # time.sleep(1)
    ...
    def generate_final_code(self, item: dict, algorithm_prompt: str, planning: str, sample_io_prompt: str, std_input_prompt: str) -> tuple[str, int, int]:
        ...
        print(input_for_final_code_generation[0]['content'])

        code, pr_tok, com_tok = self.gpt_chat(input_for_final_code_generation, index=1)
        item['api_calls'] += 1

        return self.parse_code(code), pr_tok, com_tok

    def improve_code(self, item: dict, algorithm_prompt: str, response: str, test_log: str, std_input_prompt: str) -> tuple[str, int, int]:
        ...
        print(input_for_improving_code[0]['content'])

        response, pr_tok, com_tok = self.gpt_chat(input_for_improving_code, index=2)
        item['api_calls'] += 1
