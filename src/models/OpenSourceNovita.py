...
class OpenSourceBaseModel(BaseModel):
    ...
    def __init__(
        ...
        base_url = openai_vars["base_url"] or base_url or azure_vars["base_url"]
        ...
        assert api_key is not None, "API Key must be provided as model config or environment variable (`NOVITA_API_KEY` or `AZURE_API_KEY`)"
    ...
    def read_openai_env_vars():
        return {
            ...
            "base_url": os.getenv("NOVITA_API_URL"),
...
class Llama31_8B_Novita(OpenSourceModel):
    def prompt(self, processed_input: list[dict]):
        self.model_params["model"] = "meta-llama/llama-3.1-8b-instruct"
        return super().prompt(processed_input)
    
class Llama31_70B_Novita(OpenSourceModel):
    def prompt(self, processed_input: list[dict]):
        self.model_params["model"] = "meta-llama/llama-3.1-70b-instruct"
        return super().prompt(processed_input)
    
class Qwen2_7B_Novita(OpenSourceModel):
    def prompt(self, processed_input: list[dict]):
        self.model_params["model"] = "qwen/qwen-2-7b-instruct"
        return super().prompt(processed_input)
    
class Qwen2_72B_Novita(OpenSourceModel):
    def prompt(self, processed_input: list[dict]):
        self.model_params["model"] = "qwen/qwen-2-72b-instruct"
        return super().prompt(processed_input)
    
class DeepSeekV3(OpenSourceModel):
    def prompt(self, processed_input: list[dict]):
        self.model_params["model"] = "deepseek-chat"
        return super().prompt(processed_input)
    
class DeepSeekR1(OpenSourceModel):
    def prompt(self, processed_input: list[dict]):
        self.model_params["model"] = "deepseek/deepseek-r1"
        return super().prompt(processed_input)
