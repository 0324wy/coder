...
class OpenSourceBaseModel(BaseModel):
    ...
    def __init__(
        ...
        assert api_key is not None, "API Key must be provided as model config or environment variable (`GROQ_API_KEY` or `AZURE_API_KEY`)"
        ...
        if api_type == "azure":
            ...
            self.openai = Groq(api_key=api_key,timeout=60*5)
...
class Llama31_8B_Groq(OpenSourceModel):
    def prompt(self, processed_input: list[dict]):
        self.model_params["model"] = "llama-3.1-8b-instant"
        return super().prompt(processed_input)
    
class Llama31_70B_Groq(OpenSourceModel):
    def prompt(self, processed_input: list[dict]):
        self.model_params["model"] = "llama-3.1-70b-versatile"
        return super().prompt(processed_input)